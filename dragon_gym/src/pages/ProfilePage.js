import React, { useEffect, useMemo, useRef, useState } from 'react';

function ProfilePage({ isActive }) {
  const progressTrackRef = useRef(null);
  const progressLabelRef = useRef(null);
  const [labelLeft, setLabelLeft] = useState(0);

  const barData = useMemo(
    () => [
      {
        label: 'Back',
        value: 72,
        colorClass: 'g-blue',
        textColor: 'rgba(70,155,255,.95)',
        heightScale: 0.78,
      },
      {
        label: 'Legs',
        value: 64,
        colorClass: 'g-green',
        textColor: 'rgba(67,255,150,.95)',
        heightScale: 0.82,
      },
      {
        label: 'Core',
        value: 38,
        colorClass: 'g-orange',
        textColor: 'rgba(255,194,70,.95)',
        heightScale: 0.62,
        isActive: true,
      },
      {
        label: 'Arms',
        value: 95,
        colorClass: 'g-pink',
        textColor: 'rgba(255,90,195,.95)',
        heightScale: 0.5,
      },
    ],
    []
  );

  const weeklyNow = 872;
  const weeklyGoal = 1500;
  const progressPct = Math.max(0, Math.min(1, weeklyNow / weeklyGoal));

  useEffect(() => {
    const track = progressTrackRef.current;
    const label = progressLabelRef.current;
    if (!track || !label) {
      return undefined;
    }

    const positionLabel = () => {
      const trackWidth = track.clientWidth;
      const labelWidth = label.clientWidth;
      if (!trackWidth || !labelWidth) {
        return;
      }
      const fillWidth = trackWidth * progressPct;
      const padding = 8;
      const nextLeft = Math.max(
        padding,
        Math.min(trackWidth - labelWidth - padding, fillWidth - labelWidth - 6)
      );
      setLabelLeft(nextLeft);
    };

    const frameId = window.requestAnimationFrame(positionLabel);
    window.addEventListener('resize', positionLabel);

    return () => {
      window.cancelAnimationFrame(frameId);
      window.removeEventListener('resize', positionLabel);
    };
  }, [progressPct, isActive]);

  return (
    <main className="app-shell profile-shell" hidden={!isActive}>
      <section className="profile-header" aria-label="Session summary">
        <svg
          className="profile-ghost"
          viewBox="0 0 64 64"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          aria-hidden="true"
        >
          <path
            d="M32 6c-12.2 0-22 9.8-22 22v24c0 2.2 2.6 3.3 4.2 1.8l3.5-3.2 3.7 3.3c1.1 1 2.8 1 3.9 0l3.7-3.3 3.7 3.3c1.1 1 2.8 1 3.9 0l3.7-3.3 3.7 3.3c1.1 1 2.8 1 3.9 0l3.7-3.3 3.5 3.2c1.6 1.5 4.2.4 4.2-1.8V28C54 15.8 44.2 6 32 6Z"
            fill="rgba(255,255,255,.92)"
          />
          <circle cx="24" cy="30" r="4.2" fill="rgba(0,0,0,.72)" />
          <circle cx="40" cy="30" r="4.2" fill="rgba(0,0,0,.72)" />
        </svg>
        <div className="profile-title-row">
          <h1 className="profile-title">Session Summary</h1>
          <svg
            className="profile-trophy"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
          >
            <path
              fill="#ffcc42"
              d="M18 2H6v2H3v3a5 5 0 0 0 5 5h.1A6 6 0 0 0 11 14.7V18H8v2h8v-2h-3v-3.3A6 6 0 0 0 15.9 12H16a5 5 0 0 0 5-5V4h-3V2Zm-13 5V6h1v4a3 3 0 0 1-2-3Zm16 0a3 3 0 0 1-2 3V6h2v1Z"
            />
          </svg>
        </div>
      </section>

      <section className="profile-card profile-card--summary" aria-label="User profile">
        <div className="profile-avatar">
          <img
            alt="Profile"
            src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=256&q=70"
          />
        </div>
        <div className="profile-text">
          <p className="profile-handle">@trentindeed</p>
          <div className="profile-name-row">
            <p className="profile-name">JayPowers</p>
          </div>
          <div className="profile-rank-row">
            <span>Today’s Rank</span>
            <span className="profile-rank-up">▲</span>
          </div>
          <p className="profile-rank">
            5 <span>/ 118</span>
          </p>
        </div>
      </section>

      <section className="profile-card profile-chart" aria-label="Workout volume">
        <header className="profile-chart-header">
          <p>Total Volume 389 lb</p>
        </header>
        <div className="profile-bars" role="presentation">
          {barData.map((item) => (
            <div className="profile-bar-col" key={item.label}>
              <span className="profile-bar-value" style={{ color: item.textColor }}>
                {item.value} lb
              </span>
              <div
                className={`profile-bar ${item.colorClass}`}
                style={{ height: `${Math.round(200 * item.heightScale)}px` }}
              />
              <span
                className={`profile-bar-label${item.isActive ? ' is-active' : ''}`}
              >
                {item.label}
              </span>
            </div>
          ))}
        </div>
      </section>

      <section className="profile-card profile-goal" aria-label="Weekly goal">
        <div className="profile-goal-header">
          <p>Weekly Goal</p>
          <p>1,500 lb</p>
        </div>
        <div className="profile-progress" ref={progressTrackRef}>
          <div
            className="profile-progress-fill"
            style={{ width: `${(progressPct * 100).toFixed(1)}%` }}
          />
          <span
            className="profile-progress-label"
            ref={progressLabelRef}
            style={{ left: `${labelLeft}px` }}
          >
            {weeklyNow.toLocaleString()} lb
          </span>
        </div>
      </section>
    </main>
  );
}

export default ProfilePage;
